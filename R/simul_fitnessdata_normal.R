...
#'
#' @examples
#'simul_fitnessdata_normal(seed = 1, npop = 3, nhab = 3, nrep = 5, sdpophab = 1, sdpophab_ng = 1, 
#'sigma = 0.5, rho=-1/(nhab-1), rho_ng=-1/(nhab-1)


simul_fitnessdata_normal <- function(seed = 1, npop = 3, nhab = 3, nrep = 5, sdpophab = 1, 
                                     rho = -1/(nhab-1), rho_ng = -1/(nhab-1), sdpophab_ng = 1, sigma = 0.5){
  ...
  m_perGen <- lm(fitness~pop_gen+hab_gen+Pop:Hab:IndicG0+Pop:Hab:IndicG2+SA:IndicG0+SA, data=data)
  
  m00 <- lme4::lmer(fitness~Pop+Hab+SA+(1|Pop:Hab), data=data)
  ...
  #######################################################
  ## Model to get the df of the interaction
  m_perGen <- lm(fitness~pop_gen+hab_gen+Pop:Hab:IndicG0+Pop:Hab+SA:IndicG0+SA, data=data)

  m0 <- lme4::lmer(fitness~pop_gen+hab_gen+SA+SAIndicG0+(1|Pop:Hab)+
  ...
  m1 <- aov(fitness~pop_gen+hab_gen+SA+Pop:Hab, contrasts=list(Pop="contr.sum", Hab="contr.sum"), data=data)
  ...
  m2 <- aov(fitness~pop_gen+hab_gen+SA:IndicG0+SA+Pop:Hab+Pop:Hab:IndicG0, data=data)

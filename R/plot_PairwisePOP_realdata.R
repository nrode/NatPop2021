...
plot_PairwisePOP_realdata <- function(dataset = data_PERF_Rate, trait = "Rate", gen = "G2", 
  ...
  if("Obs_A" %in% colnames(dataset)  & trait == "Nb_eggs"){
    xaxis_labelprint <- paste0("Oviposition stimulation\n in ", fruit1)
  }else{
    if("Obs_A" %in% colnames(dataset)  & trait == "Nb_adults"){
      xaxis_labelprint <- paste0("Number of adults\n in ", fruit1)
    }else{
      if("Rate" %in% colnames(dataset) && trait == "Rate"){
        xaxis_labelprint <- paste0("Egg-to-adult survival\n in ", fruit1)
      }else{
        if("BoxID" %in% colnames(dataset) && trait == "Nb_eggs"){
          xaxis_labelprint <- paste0("Oviposition preference\n in ", fruit1)
        }else{
          print("Error: unknown combinaison dataset x trait")
        }
      }

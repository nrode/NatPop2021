...
datablank <- data.frame(Mean = c(1,1,1), 
                       Test_environment = c("Blackberry","Cherry","Strawberry"))


plot_blank <- ggplot(datablank) + 
  geom_bar(aes(Mean, fill = Test_environment)) + 
  geom_point(aes(x = Mean, y = Mean, shape =  Test_environment), size=3) + 
...
legend_all <- lemon::g_legend(plot_blank)

...
```{r }
#Check: 
sim <- sapply(1:1000, simul_powertest_nongenet, SAng= 0.032)
...
#SA and SAnon genet estimated with simulated data are similar than 
  #SA and SAnon genet estimated with real data
...
```{r }
#nb_simul = 500
val_SAng <- seq(from = 0, to = 1, by = 0.05)
...
data_prop <- data.frame(val_SAng = val_SAng, 
                 prop_gen_sign = tapply(sim$IndicGen, 
                                        sim$SAng, mean), 
                 prop_nongen_sign = tapply(sim$IndicNonGen, 
                                        sim$SAng, mean))
data_prop

#Plot des simuls
power_test <- ggplot2::ggplot(data = data_prop, aes(x=val_SAng)) + 
  geom_point(aes(y = prop_nongen_sign), size = 3) + 
  geom_line(aes(y = prop_nongen_sign), size = 1) + 

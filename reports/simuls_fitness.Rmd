...
#Perform siluations
sim <- sapply(1:nb_simul, simul_fitnessdata, distrib = "normal", rho = -0.05, rho_ng = -0.05, 
...
plot_genetic_normal<-ggplot2::ggplot(data = data_prop, aes(x=val_SA_genet)) + 
   ...
   geom_line(aes(y=prop_val_sign, color="LMER"), size = 1.5) + 
   geom_point(aes(y=prop_val_sign_aov, color="Anova"), size = 2.5) + 
   geom_line(aes(y=prop_val_sign_aov, color="Anova"), size = 0.75) + 
...
#Perform siluations
sim <- sapply(1:nb_simul, simul_fitnessdata, distrib = "binomial",  rho = -0.05, rho_ng = -0.05, 
              npop_per_fruit = 5, nfruit = 3, nhab = 3, nrep = 10, sdpop = 0.5, 
...
#Simul data 
nb_simul <- 10000
sim <- sapply(distrib = "normal", data = data_PERF, seed = 1, sdpop = 0.5, 

...
plot_genetic<-ggplot2::ggplot(data = data_prop, aes(x=val_SA_genet)) + 
   geom_point(aes(y=prop_val_sign), color="#DCA237", size = 3) + 
   geom_line(aes(y=prop_val_sign), color="#DCA237", size = 1, linetype = "dashed") + 
   geom_point(aes(y=prop_val_sign_aov), color="#459B76", size = 3) + 
   geom_line(aes(y=prop_val_sign_aov), color="#459B76", size = 1) + 
   ylab("Proportion of simulations with\nsignificant genetic local adaption")  +
   xlab("Intensity of genetic local adaptation (SA)")  +
   theme_LO_sober  + 
   labs(color = "Models") +
   scale_color_manual(values = c("LMER" = "#DCA237", "Anova" = "#459B76"))
plot_genetic


#Save plot
name_plot<-paste0("Simul_powertest_genetic_",
...
#Save plot
name_plot<-paste0("Simul_powertest_nongenetic_",
      nb_simul,"nbsimul_",nb_per_SA,
      "nbsimulperSA.pdf")
cowplot::save_plot(file =here::here("figures",name_plot ),
...
```{r } 
sim <- sapply(1:5, simul_fitnessdata_normal, rho=0, rho_ng=0, sigma = 0.5, nrep=10)
sim <- as.data.frame(t(sim))
sim <- add_indic_sign(sim)
...
sim <- sapply(1:500, simul_fitnessdata_normal, rho=-0.1, rho_ng=0, sigma = 0.5, nrep=10)
sim <- as.data.frame(t(sim))
sim <- add_indic_sign(sim)
...
sim <- sapply(1:500, simul_fitnessdata_normal, rho=0, rho_ng=-0.1, sigma=0.5, nrep=10)
sim <- as.data.frame(t(sim))
sim <- add_indic_sign(sim)

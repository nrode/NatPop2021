...
data_prop <- data.frame(val_SA_genet = sort(unique(sim_select_genetic$SA_Gen)), 
                 ...
                 prop_val_sign_aov = tapply(sim_select_genetic$IndicGen_aov, 
                                        sim_select_genetic$SA_Gen, mean))
data_prop$design <- 1
data_prop$design <- "Slighly unbalanced"
...
data_prop <- data.frame(val_SA_genet = sort(unique(sim_select_genetic$SA_Gen)), 
                 ...
                 prop_val_sign_aov = tapply(sim_select_genetic$IndicGen_aov, 
                                        sim_select_genetic$SA_Gen, mean))
data_prop$design <- "Highly unbalanced"
data_prop_gen <- rbind(data_prop_gen,data_prop)
...
## Proportion of simulations with significant genetic local adaptation for each value of SA
tapply(sim$IndicGen, sim$SA_Gen, mean)
tapply(sim$IndicGen_aov, sim$SA_Gen, mean)

## Test significant when proportion of pop hab variance explained by SA is greater than 70%
tapply(sim$IndicGen_aov, sim$SA_Gen, length)

## Proportion of simulations with significant genetic local adaptation for each value of non genetic SA
tapply(sim$IndicGen, sim$SA_NonGen, mean)
tapply(sim$IndicGen_aov, sim$SA_NonGen, mean)

plot(sim$rsqgen~sim$SA_Gen)

## Proportion of significant test increases with the proportion of non-genetic pop hab variance explained by non-genetic SA
## Above the expected 5% threshold when proportion of on-genetic pop hab variance explained by non-genetic SA is greater than 50%
tapply(sim$IndicGen_aov, sim$SA_NonGen, mean)

## We can see that rsqgen is positively correlated to rsqng or SA_NonGen
boxplot(sim$rsqgen~sim$SA_NonGen)
boxplot(sim$rsqgen~sim$SA_NonGen)
...
boxplot(sim$rsqng~sim$SA_NonGen)
...
plot_genetic_poisson_ALL_box<-ggplot2::ggplot(data = data_proportion_gen, 
   ...
   ylab("Proportion of simulations with\nsignificant local adaption")  +
   labs(x=expression(SAgenetic["G0G2"]))  +
   ggtitle("Poisson data") +
   theme_LO_sober  + 
...
plot_nongenetic_poisson_ALL_box<-ggplot2::ggplot(data = data_proportion_nongen, 
   ...
   geom_line(size = 1) + 
   ylab("Proportion of simulations with\nsignificant adaptive phenotypic plasticity")  +
   labs(x=expression(SAplastic["G0"]))  +
   ggtitle("Poisson data") +
   theme_LO_sober  + 
   labs(color = "Methods", linetype = "Box effect\n(sd box)") +
   scale_color_manual(values = c("#459B76","#DCA237")) +

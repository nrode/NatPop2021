...
head(data_PERF)
tapply(data_PERF$Nb_adults,list(data_PERF$Original_environment,
                                data_PERF$Generation),length)
tapply(data_PERF$Nb_adults,list(data_PERF$Original_environment,data_PERF$Test_environment,
                                data_PERF$Generation),length)
data_PERF$Date_P=as.Date(data_PERF$Date_P, format="%d/%m/%Y")
data_PERF$Date_P <- data_PERF$Date_P + lubridate::years(2000)
#############################
...
data_PERF_Rate$Date_P <- data_PERF_Rate$Date_P + lubridate::years(2000)
###########################
#PREFERENCE 
###########################
...
wt3rate<-ggplot(data = data_PERF_Rate,
  ...
  ylab("Residuals(offspring performance)")  +
  xlab("Date")  +
...
# m1 <- lme4::lmer(resid ~ (1|Date_P), data = data_PERF_Rate, REML= TRUE)
# summary(m1)

m1 <- lme4::lmer(asin(sqrt(Rate)) ~ Test_environment+ log(Nb_eggs) + (1|Date_P), data = data_PERF_Rate)
m2 <- lm(asin(sqrt(Rate)) ~ Test_environment+ log(Nb_eggs), data = data_PERF_Rate)
anova(m1, m2)
...
```{r }
#Extract residuals Rate~Treatment 
m1 <- aov(log(Nb_eggs+1) ~ Test_environment, data = data_PERF)
data_PERF$resid_eggs <- residuals(m1)


wt3eggs<-ggplot(data = data_PERF,
                aes(x = Date_P, y = resid_eggs)) + 
  geom_point(size = 1.5, alpha = 0.7) +
  ylab("Residuals(fecundity)")  +
  scale_x_date(date_breaks = "1 month", 
                 limits = as.Date(c('2018-06-01','2018-12-01'))) + 
  xlab("Date")  +
  ylim(-2,2) +
...
#Variance intra date
m1 <- lme4::lmer(log(Nb_eggs+1) ~ Test_environment + (1|Date_P), data = data_PERF)
summary(m1)
#m1 <- lme4::lmer(resid_eggs ~ 1 + (1|Date_P), data = data_PERF_Rate)
#summary(m1)


m1 <- lme4::lmer(log(Nb_eggs+1) ~ Test_environment + (1|Date_P), data = data_PERF_Rate,  REML=TRUE)
m0 <- lm(log(Nb_eggs+1) ~ Test_environment , data = data_PERF_Rate)
#AIC(m0,m1,m2)
...
#Variance intra date
m1 <- lme4::lmer(log(Nb_eggs+1) ~ Test_environment  + (1|Date_P)+ (1|BoxID), 
                 data = data_PREF)
summary(m1)
...
m2 <- lme4::lmer(log(Nb_eggs+1) ~ Test_environment + (1|BoxID), data = data_PREF, 
                 REML=TRUE)
...
cowplot::save_plot(file = here::here("figures", "Supplements_FigS1_WT3.pdf"),
...
```{r }
wt3rate<-ggplot(data = data_PERF_Rate,
                aes(x = Date_P, y = Rate, color = Test_environment)) + 
  geom_point(size = 1.5, alpha = 0.7) +
  ylab("Offspring performance")  +
  xlab("Date")  +
  scale_x_date(date_breaks = "1 month", 
                 limits = as.Date(c('2018-06-01','2018-12-01'))) + 
    guides(col=guide_legend("Test fruit")) + 
 theme_LO_sober
wt3rate
...
wt3eggs<-ggplot(data = data_PERF,
                aes(x = Date_P, y = Nb_eggs, color = Test_environment)) + 
  geom_point(size = 1.5, alpha = 0.7) +
  ylab("Fecundity")  +
  scale_x_date(date_breaks = "1 month", 
                 limits = as.Date(c('2018-06-01','2018-12-01'))) + 
...
cowplot::save_plot(file = here::here("figures", "Supplements_FigS1_WT3_RawData.pdf"),
                   THREE_JOIN, base_height = 28/cm(1), base_width = 19/cm(1), dpi = 1200)
#

...
head(data_PERF_Rate)
library(lme4)
data_PERF_Rate$SAneg <- as.factor(ifelse(data_PERF_Rate$Original_environment==data_PERF_Rate$Test_environment, 1, 0))

m <- lmer(asin(sqrt(Rate))~ SAneg + SAneg*IndicG0 + Test_environment:Generation  + (1|Population:Test_environment) + (0 + dummy(Generation, "G0")|Population:Test_environment), data=data_PERF_Rate[!is.na(data_PERF_Rate$Rate),])
summary(m)

data.frame(VarCorr(m))$vcov
fixef(m)


M1 <- model.matrix(~SA + SA*IndicG0 + Test_environment:Generation, data=data_PERF_Rate[!is.na(data_PERF_Rate$Rate),])
dim(M1)
M1 <- M1[, colnames(M1)%in%names(fixef(m))]
dim(M1)

data.frame(predict(m, re.form = NA), M1%*%fixef(m) )
